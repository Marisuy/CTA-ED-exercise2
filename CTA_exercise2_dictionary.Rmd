...
tweets <-
  get_all_tweets(
    ...
    data_path = "Users/aitingrnb/Downloads/",
    n = Inf,
  )

tweets  <- readRDS(gzcon(url(")))
```
...
tidy_dfm_trans <- toks_news %>%
  tokens_lookup(dictionary = trans_dict) %>%   # 1. 查找词典
  dfm() %>%                                    # 2. 生成 DFM 对象
  dfm_group(groups = newspaper) %>%            # 3. 分组 (这步必须在 convert 之前！)
  convert(to = "data.frame") %>%               # 4. 最后才转成 data.frame
...
# 1. 先把数据从“宽表”转成“长表”
tidy_plot_data <- tidy_dfm_trans %>%
  pivot_longer(cols = c(trans, terf), names_to = "category", values_to = "count")
ggplot(tidy_plot_data, aes(x = reorder(newspaper, count), y = count, fill = category)) + 
  geom_col() +            # 使用柱状图 (geom_col) 比散点图 (geom_point) 更适合展示词频
  coord_flip() +          # 横向翻转
  xlab("Newspapers") + 
...
tidy_tweets %>%
  ...
  geom_point(alpha=0.5) +
  geom_smooth(method= loess, alpha=0.25) +
  xlab("Date") + ylab("% trans-related terms")
