...
toks_news_small <- toks_news[1:5000]#computer burnout
toks_news_lsd <- tokens_lookup(toks_news_small, dictionary = data_dictionary_LSD2015_pos_neg)
```
...
```{r}
dfm_newspaper_lsd <- toks_news_lsd %>% 
  dfm()%>%
  dfm_group(groups = username)

tidy_newspaper_lsd <- dfm_newspaper_lsd %>%
  convert(to = "data.frame") %>%
  rename("newspaper" = doc_id) %>%
  mutate(overall_sentiment = positive - negative)

ggplot(tidy_newspaper_lsd, aes(x = reorder(newspaper, overall_sentiment), y = overall_sentiment)) + 
  geom_col(fill = "steelblue") + 
  coord_flip() + 
  labs(
    title = "Overall Sentiment by Newspaper (LSD Dictionary)", 
    x = "Newspaper", 
