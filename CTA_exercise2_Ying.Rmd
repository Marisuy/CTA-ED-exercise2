...
```
#?raw=true 下载原始数据文件，而不是看网页
#gzcon(...)	处理数据的压缩流
#readRDS(...)	将下载的数据读入 R 中，并赋值给 tweets
...
colnames(tweets)
```
#head(tweets)	内容抽查：显示数据的前 6 行
#colnames(tweets) 结构检查：列出所有的列名（变量名）
...
```
#echo = F (False)在生成的报告中隐藏这段代码，只最后生成的表格
#排序：arrange(created_at) 按照“发布时间”对数据进行排序。默认是升序，从旧到新
#筛选：tail(5)截取最后 5 行
#制作表格：kbl()
#美化样式：kable_styling(...)；striped: 斑马纹（一行深一行浅，方便阅读）；hover: 鼠标悬停变色效果。；condensed: 紧凑型排版（缩小行间距，更节省空间）。；responsive: 响应式（在手机或不同大小屏幕上都能自适应显示）
...
#数据清洗与分词
#mutate创建desc的新列
#unnest_tokens把desc列中的句子拆散，一行一词
#filter过滤，只保留包含英文字母的单词
...
## Get sentiment dictionaries 获取词典
...
```
#get_sentiments("nrc") 从 tidytext 包中调取 NRC 词典（词典中每个词都标注了情感分类）
#inner_join() 内部连接（取交集）
#count(word, sort = TRUE)计数并且从高到低排列
...
```
#as.Date(...)转换函数，把括号里的文字看作日期
#arrange(date)排序函数，根据 date列把所有行重新排列。
#nrow(tidy_tweets)查询函数，用来计算 tidy_tweets 表里一共有多少行
#1:nrow(...)； : 符号表示创建一个连续的数字序列。
...
```
#index = order %/% 1000。%/% 是整除符号。比如：1500/1000 = 1.5，那么 $1500 %/% 1000 = 1。逻辑：每 1000 个单词被归为一个“组（index）”。目的：单条推文太短，把 1000 个单词聚在一起看，能看清大趋势
#sentiment: 统计每种情感（如 positive, negative）分别出现了几次。
#spread(): 长表转宽表（透视）。原本所有的标签（positive, negative, fear...）都在一列里，现在它们变成了独立的列。
#fill = 0: 如果某个时间段里一个“恐惧”词都没有，就填 0，而不是留白（NA）。
#geom_smooth() 添加一条平滑趋势线。找出真正的规律。
#method = loess含义：指定平滑算法。loess 代表“局部加权回归”。
#alpha = 0.25 透明度通常作用于曲线周围的“置信区间阴影”（淡灰色的区域）。它代表了统计上的不确定性：阴影越窄，说明趋势越可信。
...
#get total tweets per day (no missing dates so no date completion required) 计算分母
totals <- tidy_tweets %>%
  mutate(obs=1) %>%
  group_by(date) %>%
...
```
#collapse = "；在正则表达式中，符号 | 代表 “或” (OR)。
#grepl：逻辑判断函数。在 word 列中寻找匹配这些名单的单词
#不区分大小写（ignore.case = T）
# full_join(totals, by="date")：将“死亡词统计表”和“每日总词数表”合并。
# ifelse(is.na(...), 0, ...)：如果某天没有死亡词（显示为 NA），就把它填为 0。
# pctmwords = sum_mwords / sum_words：核心计算。分子 ÷ 分母 = 死亡词百分比
...
```
#data_dictionary_LSD2015：这是 quanteda 自带的一个经典词典，全称是 Lexicoder Sentiment Dictionary。它专门为政治和新闻文本设计。
#[1:2]：索引操作。创建一个只包含好和坏的子词典。原始的 LSD2015 词典通常包含四个类别：negative（消极）、positive（积极）、neg_negative（否定消极）和 neg_positive（否定积极）。这一行代码的意思是：只选取前两个类别，即纯粹的消极和积极词汇，忽略掉那些复杂的双重否定逻辑。
...
```
#dfm(): 创建文档-特征矩阵
#matplot(): Base R 中的矩阵绘图函数。 特点：它可以一次性把矩阵里的多列（积极和消极）同时画出来。
# type = "l": 画成折线图（Line）。
# col = 1:2: 第一列（通常是消极）用黑色，第二列（积极）用红色（取决于词典顺序）。
# grid(): 在图表背景添加网格线，方便观察数值。
# legend(...): 添加图例。告诉读者哪条线代表 positive，哪条线代表 negative。
...
```
# @ 符号用来访问其内部的槽位。x 存放的是具体的频数。
# Dimnames$docs: 从矩阵的维度名称中提取“文档名”。
# cbind(...): 意为列合并。
# as.data.frame(...): 将合并后的结果强制转换成标准的数据框格式
# as.numeric: 数据转回数值型
# as.Date: 将日期列转回日期对象
...
# 以“user_name”为例，这些名称描述了推特账号的报纸来源。如果只看不同的报纸来源，我们会看到不同的情绪动态吗？
```{r}
...
```
# docvars()：这是 quanteda 包里查看文档变量（元数据）的函数
# docvars(toks_news)：取出存放所有附加信息的表格。
# $username：锁定表格中名为 username 的那一列（代表报纸名称）。
# unique()：这是一个去重函数
...
# Answer: Yes, we can see different sentiment dynamics. The charts illustrate the sentiment score trends for eight major British newspapers on Twitter from January to May 2020. According to the summary dot plot, there is a clear divide between different newspaper. The Daily Mirror exhibited the most extreme negative sentiment. Other newspapers, such as Daily Mail UK and The Sun, also showed high levels of negativity, though significantly less than DailyMirror. Conversely, MetroUK, thetimes, and Telegraph remained closest to the neutral zero-line, indicating a more balanced or restrained use of emotional language. 
# Moreover, there are dynamic trends over time. While The Sun and Daily Mirror showed a recovery starting in April, other newspapers like guardian maintained a much stable trends throughout the period. In conclusion, different newspaper sources indeed display distinct sentiment dynamics.
...
totals_newspaper_clean <- totals_newspaper %>%
  mutate(newspaper = tolower(gsub("[[:space:][:punct:]]|the", "", newspaper))) %>%
  group_by(newspaper) %>%
  summarise(sum_words = sum(sum_words))

tidy_dfm_trans <- toks_news %>%
  tokens_lookup(dictionary = trans_dict) %>%
  dfm() %>%
  dfm_group(groups = username) %>%
  convert(to = "data.frame") %>%
  rename("newspaper" = doc_id) %>%
  mutate(newspaper = tolower(gsub("[[:space:][:punct:]]|the", "", newspaper))) %>%
  full_join(totals_newspaper_clean, by="newspaper") %>%
  mutate(
    sum_mwords = trans + terf,
    sum_mwords = ifelse(is.na(sum_mwords), 0, sum_mwords),
    pcttranswords = sum_mwords / sum_words
  ) %>%
  filter(!is.na(pcttranswords) & sum_words > 0)

tidy_dfm_trans %>%
  ggplot() +
  geom_point(aes(x = reorder(newspaper, pcttranswords), y = pcttranswords), size = 3) +
  coord_flip() +
  xlab("Newspapers") +
  ylab("% words referring to trans or terfs") +
  theme_minimal()
```
...
```{r}
# --- 1. 加载字典 | Load the Lexicoder Sentiment Dictionary ---
# quanteda 自带了 LSD2015 字典
lsd_dict <- data_dictionary_LSD2015

# --- 2. 核心分析流水线 | Core Sentiment Analysis Pipeline ---
# 逻辑：分词 -> 查情感字典 -> 生成矩阵 -> 转换为表格 -> 计算得分
sentiment_by_newspaper <- toks_news %>%
  tokens_lookup(dictionary = lsd_dict) %>% # 使用 LSD 字典查找情感词
  dfm() %>% 
  dfm_group(groups = username) %>%         # 按报纸（username）分组
  convert(to = "data.frame") %>%           # 转换为数据框
  rename("newspaper" = doc_id) %>% 
  # 计算情感得分 (公式：正面词总和 - 负面词总和)
  # LSD2015 包含 positive, negative, neg_positive (被否定的正面词), neg_negative (被否定的负面词)
  mutate(
    # 统一清洗报纸名称，防止出现重复行
    newspaper = tolower(gsub("[[:space:][:punct:]]", "", newspaper)),
    # 计算净情感得分：(正面 + 否定负面) - (负面 + 否定正面)
    net_sentiment = (positive + neg_negative) - (negative + neg_positive)
  )

# --- 3. 结果可视化 | Visualization ---
sentiment_by_newspaper %>%
  ggplot(aes(x = reorder(newspaper, net_sentiment), y = net_sentiment)) +
  geom_point(size = 2) + 
  coord_flip() + # 翻转坐标轴便于阅读
  labs(
    title = "Overall Sentiment by Newspaper (LSD2015)",
    subtitle = "Calculated as: (Pos + Neg_Neg) - (Neg + Neg_Pos)",
    x = "Newspaper Source",
